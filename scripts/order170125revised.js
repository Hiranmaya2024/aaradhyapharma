document.addEventListener('DOMContentLoaded', async function() {
    ...
    //const shopName = sessionStorage.getItem('shop-name');
   // const customername = sessionStorage.getItem('customer-name');
    ...
    confirmOrderButton.addEventListener('click', async () => {
        ...
        cart.forEach(item => {
            ...
            // Group by company
            const companyName = stockData.find(row => row[1] === item.name)[0];
            if (!companyWiseData[companyName]) {
                companyWiseData[companyName] = { itemCount: 0, preGstAmount: 0, netAmount: 0 };
            }
            companyWiseData[companyName].itemCount += item.quantity;
            companyWiseData[companyName].preGstAmount += itemTotal;
            companyWiseData[companyName].netAmount += itemTotalWithGst;
        });
        ...
        yPosition += 20;
        doc.text('Company Wise Sale', 10, yPosition);
        yPosition += 10;
        doc.text('Company Name', 10, yPosition);
        doc.text('Item Count', 60, yPosition);
        doc.text('Pre-GST Amount', 110, yPosition);
        doc.text('Net Amount', 160, yPosition);
        yPosition += 10;
        Object.keys(companyWiseData).forEach(companyName => {
            doc.text(companyName, 10, yPosition);
            doc.text(companyWiseData[companyName].itemCount.toString(), 60, yPosition);
            doc.text(companyWiseData[companyName].preGstAmount.toFixed(2).toString(), 110, yPosition);
            doc.text(companyWiseData[companyName].netAmount.toFixed(2).toString(), 160, yPosition);
            yPosition += 10;
        });
